...
var FocusScope = React4.forwardRef(
  __name4(function FocusScope2(props, forwardedRef) {
    ...
    const branchesRef = React4.useRef(branches);
    React4.useEffect(() => {
      branchesRef.current = branches;
    });
    const isTargetInScope = React4.useCallback(
      (target) => {
        var _a;
        if (!target) return false;
        if (container == null ? void 0 : container.contains(target)) return true;
        return Boolean((_a = branchesRef.current) == null ? void 0 : _a.some((branch) => branch.contains(target)));
      },
      [container]
    );
...
var FocusScopeBranchContext = React4.createContext(null);
...
function useFocusScopeBranchRegistry() {
  ...
  const registry = React4.useMemo(
    () => ({
      add: __name4((node) => setNodes((prev) => prev.includes(node) ? prev : [...prev, node]), "add"),
      remove: __name4((node) => setNodes((prev) => prev.filter((current) => current !== node)), "remove")
    }),
    []
  );
  return { nodes, registry };
}
__name4(useFocusScopeBranchRegistry, "useFocusScopeBranchRegistry");
function useFocusScopeBranch(node) {
  const registry = React4.useContext(FocusScopeBranchContext);
  React4.useEffect(() => {
...
__name4(useFocusScopeBranch, "useFocusScopeBranch");
...
//# sourceMappingURL=chunk-ADUFTTDE.js.map
